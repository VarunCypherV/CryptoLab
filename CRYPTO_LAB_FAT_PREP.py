...
Ya = (root ** alicesecret) % prime     #A-->ATTACKER
# Attacker intercepts Party2's public key
Yb = (root ** bobsecret) % prime      #B --> ATTACKER
Ka = (root ** Xd1) % prime #sent to bob
Kb = (root ** Xd2) % prime #sent to alice
...
def mixColumns(state, inverse=False):
    for i in range(4):
        if inverse:
            s0 = (mul(state[0][i], 0x0e) ^ mul(state[1][i], 0x0b) ^ mul(state[2][i], 0x0d) ^ mul(state[3][i], 0x09)) & 0xff
            s1 = (mul(state[0][i], 0x09) ^ mul(state[1][i], 0x0e) ^ mul(state[2][i], 0x0b) ^ mul(state[3][i], 0x0d)) & 0xff
            s2 = (mul(state[0][i], 0x0d) ^ mul(state[1][i], 0x09) ^ mul(state[2][i], 0x0e) ^ mul(state[3][i], 0x0b)) & 0xff
            s3 = (mul(state[0][i], 0x0b) ^ mul(state[1][i], 0x0d) ^ mul(state[2][i], 0x09) ^ mul(state[3][i], 0x0e)) & 0xff
        else:
            s0 = (mul(state[0][i], 0x02) ^ mul(state[1][i], 0x03) ^ state[2][i] ^ state[3][i]) & 0xff
            s1 = (state[0][i] ^ mul(state[1][i], 0x02) ^ mul(state[2][i], 0x03) ^ state[3][i]) & 0xff
            s2 = (state[0][i] ^ state[1][i] ^ mul(state[2][i], 0x02) ^ mul(state[3][i], 0x03)) & 0xff
            s3 = (mul(state[0][i], 0x03) ^ state[1][i] ^ state[2][i] ^ mul(state[3][i], 0x02)) & 0xff
        state[0][i] = s0
        state[1][i] = s1
        state[2][i] = s2
        state[3][i] = s3
